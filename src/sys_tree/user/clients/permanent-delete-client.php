...
if ($is_deleted) {
  ...
  $is_exist = $pcs_obj->is_exist("`id`", "`deleted_pieces_info`", $client_id);
  // get client name
  $client_name = $is_exist ? $pcs_obj->select_specific_column("`full_name`", "`deleted_pieces_info`", "WHERE `id` = $client_id")[0]['full_name'] : null;
} else {
  // get user info from database
...
if ($_SESSION['sys']['isLicenseExpired'] == 0) {
  if ($is_exist == true) {
    ...
    if ($is_exist > 0 && $count_child == 0) {
      ...
      $is_deleted = $pcs_obj->permanent_delete_client($client_id);
