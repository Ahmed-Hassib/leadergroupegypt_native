    ...
    <div class="hstack gap-2">
      <?php if ($_SESSION['sys']['user_add'] == 1) { ?>
        ...
        <button type="button" form="add-new-user" class="btn btn-primary text-capitalize py-1 fs-12 <?php echo @$_SESSION['sys']['lang'] == 'ar' ? 'me-auto' : 'ms-auto' ?>" onclick="form_validation(this.form, 'submit')">
          <span><?php echo lang('ADD NEW', $lang_file) ?></span>&nbsp;<i class="bi bi-person-plus"></i>
        </button>
      <?php } ?>
      <?php if ($_SESSION['sys']['user_show'] == 1) { ?>
        <!-- submit button -->
        <a href="?do=manage" class="btn btn-outline-secondary text-capitalize py-1 fs-12">
          <span><?php echo lang('ALL EMPLOYEES', $lang_file) ?></span>&nbsp;<i class="bi bi-people"></i>
        </a>
      <?php } ?>
    </div>
    <!-- end submit -->
          ...
          <div class="mb-3 position-relative">
            <div class="form-floating form-floating-<?php echo $_SESSION['sys']['lang'] == 'ar' ? 'right' : 'left' ?>">
              <select class="form-select" name="job_title_id" id="job_title_id" <?php echo $_SESSION['sys']['user_update'] == 0 ? 'disabled' : '' ?> required>
                <option value="default" selected disabled><?php echo lang('JOB TITLE', $lang_file) ?></option>
                <?php
                if (!isset($db_obj)) {
                ...
                <?php foreach ($job_titles as $job_title) { ?>
                  <option value="<?php echo $job_title['job_title_id'] ?>"><?php echo lang($job_title['job_title_name'], $lang_file) ?></option>
                <?php } ?>
              </select>
              <label for="job-title"><?php echo lang('JOB TITLE', $lang_file) ?></label>
            </div>
