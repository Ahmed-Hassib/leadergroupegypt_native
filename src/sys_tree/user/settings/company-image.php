  ...
  <div class="company-img-container" id="company-image-container">
    ...
    <form action="?do=change-company-img" method="POST" id="change-company-image" enctype="multipart/form-data">
      ...
      <input type="file" class="d-none" name="company-img-input" id="company-img-input"
        onchange="change_company_img(this)" accept="image/*">
    </form>
    <?php if (empty($_SESSION['sys']['company_img'])) { ?>
      <span class="text-center text-muted" id="company-img-status">
    ...
    <div class="company-img-btn company-img-btn-<?php echo $_SESSION['sys']['lang'] == 'ar' ? 'left' : 'right' ?>">
      ...
      <?php if (!empty($_SESSION['sys']['company_img'])) { ?>
        ...
        <button type="button" role="button" class="btn btn-danger fs-12 py-1 text-capitalize"
          onclick="delete_company_image('company')">
          <i class="bi bi-trash"></i>
          <?php echo lang('DELETE') ?>
        </button>
      <?php } ?>

      <button type="submit" class="btn btn-success fs-12 py-1 text-capitalize d-none" form="change-company-image"
        id="change-company-img-btn">
