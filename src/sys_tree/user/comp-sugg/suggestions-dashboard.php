...
// get all suggestions
$all_suggestions = $comp_sugg_obj->get_all_suggestions($_SESSION['UserID'], $_SESSION['company_id']);
?>
<div class="container mb-0" dir="<?php echo @$_SESSION['systemLang'] == 'ar' ? 'rtl' : 'ltr' ?>">
  <div class="mb-3">
    <a href="?do=add-comp-sugg&type=1" class="btn btn-outline-primary py-1 fs-12 shadow-sm">
      ...
      <?php echo language('ADD NEW SUGGESTION', @$_SESSION['systemLang']) ?>
    </a>
  </div>
</div>
<!-- start new design -->
    ...
    <div class="col-12">
      <div class="section-block">
        <header class="section-header">
          <h5 class="h5 text-capitalize"><?php echo language('TOTAL SUGGESTIONS', @$_SESSION['systemLang']) ?></h5>
          <hr>
        </header>
            ...
            <div class="card card-stat card card-stat shadow-sm border border-1">
              <div class="card-body">
                ...
                <span class="bg-info icon-container <?php echo @$_SESSION['systemLang'] == 'ar' ? 'icon-container-left' : 'icon-container-right' ?>">
                  <span class="nums">
                    <?php $all_suggestions = $comp_sugg_obj->count_records("`id`", "`comp_sugg`", "WHERE `type` = 1 AND `company_id` = " . $_SESSION['company_id'] . " AND `added_by` = " . $_SESSION['UserID']); ?>
