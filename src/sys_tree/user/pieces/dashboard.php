      ...
      <div class="col-sm-12 col-lg-5">
        <div class="section-block">
          <div class="section-header">
            ...
            <p class="text-muted ">
              <?php echo language("HERE WILL SHOW SOME STATISTICS ABOUT THE NUMBERS OF PIECES", @$_SESSION['systemLang']) ?>
            </p>
            <hr>
          </div>
              ...
              <div class="card card-stat bg-primary shadow-sm border border-1">
                <div class="card-body">
                  ...
                  <h5 class="card-title text-capitalize">
                    <?php echo language('TOTAL', @$_SESSION['systemLang']) . " " . language('PIECES', @$_SESSION['systemLang']) ?>
                  </h5>
                  <span
                    class="bg-info icon-container <?php echo @$_SESSION['systemLang'] == 'ar' ? 'icon-container-left' : 'icon-container-right' ?>"><i
      ...
      <div class="col-12">
        <div class="section-block">
          <div class="section-header">
            ...
            <p class="text-muted ">
              <?php echo language('HERE WILL SHOW LATEST 10 ADDED PIECES', @$_SESSION['systemLang']) ?></p>
            <hr>
          </div>
            ...
            <table class="table table-striped table-bordered  display compact table-style w-100" id="latest-pieces">
              <thead class="primary text-capitalize">
                <tr>
                  ...
                  <th style="min-width: 300px" class="text-uppercase">
                    <?php echo language('IP', @$_SESSION['systemLang']) ?></th>
                ...
                <tr>
                  ...
                  <td class="text-capitalize" data-ip="<?php echo convert_ip($pcs['ip']) ?>">
                    ...
                    <span class="device-status">
                      ...
                      <span class="pcs-ip" data-pcs-ip="<?php echo $pcs['ip'] ?>"><?php echo $pcs['ip'] ?></span>
                    </span>
                    ...
                    <button class="btn btn-outline-primary fs-12 px-3 py-1" data-bs-toggle="modal"
                      ...
                      onclick="ping('<?php echo $pcs['ip'] ?>', <?php echo $_SESSION['ping_counter'] ?>)">ping</button>
                    <?php } ?>
                  </td>
                  <!-- piece name -->
                  <td>
                  ...
                  <td class="text-capitalize">
                    <?php if ($pcs['direction_id'] != 0) { ?>
                    <a
                      href="<?php echo $nav_up_level ?>directions/index.php?do=show-direction-tree&dir-id=<?php echo $pcs['direction_id']; ?>">
                  ...
                  <!-- added date -->
                  <td>
                    <?php echo $pcs['added_date'] == '0000-00-00' ? language("NO DATA ENTERED", @$_SESSION['systemLang']) : $pcs['added_date'] ?>
                  </td>
                  <!-- control -->
                  <td>
                    <?php if ($_SESSION['pcs_update'] == 1) { ?>
                    <a class="btn btn-success text-capitalize fs-12 py-1"
                      href="?do=edit-piece&piece-id=<?php echo $pcs['id']; ?>" target=""><i
                        class="bi bi-pencil-square"></i>
                      <!-- <?php echo language('EDIT', @$_SESSION['systemLang']) ?> --></a>
                    <?php } ?>
                    <?php if ($_SESSION['pcs_show'] == 1) { ?>
                    <a class="btn btn-outline-primary text-capitalize fs-12 py-1"
                      href="?do=show-piece&dir-id=<?php echo $pcs['direction_id'] ?>&src-id=<?php echo $pcs['id'] ?>"><i
                        class="bi bi-eye"></i>
                      <!-- <?php echo language('SHOW', @$_SESSION['systemLang']) . ' ' . language('PIECES', @$_SESSION['systemLang']) ?> --></a>
                    <?php } ?>
                    <?php if ($_SESSION['pcs_delete'] == 1) { ?>
                    <button type="button"
                      class="btn btn-outline-danger text-capitalize form-control bg-gradient fs-12 py-1"
                      data-bs-toggle="modal" data-bs-target="#deletePieceModal" id="delete-piece"
                      data-page-title="<?php echo $page_title ?>" data-piece-id="<?php echo $pcs['id'] ?>"
                      data-piece-name="<?php echo $pcs['full_name'] ?>" onclick="confirm_delete_piece(this, true)"><i
