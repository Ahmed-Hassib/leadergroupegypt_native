  ...
  <div class="modal-dialog modal-dialog-centered modal-company-dialog" dir="<?php echo $page_dir ?>">
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="m-auto modal-title h5 " id="staticBackdropLabel"><?php echo lang("ADD DEVICE", $lang_file) ?></h5>
      </div>
            ...
            <?php } else { ?>
              <select class="form-select" name="company-id" id="company-id" required>
                <?php if (count($manufacture_companies) > 0 && $manufacture_companies != null) { ?>
                  <option value="default" disabled selected><?php echo lang('SELECT MAN COMPANY', $lang_file) ?></option>
                  <?php foreach ($manufacture_companies as $company) { ?>
                    <option value="<?php echo base64_encode($company['man_company_id']) ?>"><?php echo $company['man_company_name'] ?></option>
                  <?php } ?>
                <?php } ?>
              </select>
            <?php } ?>
            <label for="<?php echo isset($dev_company_name) ? 'company-name' : 'company-id' ?>"><?php echo lang('COMPANY NAME', $lang_file) ?></label>
            <?php if (count($manufacture_companies) == 0 && $manufacture_companies == null) { ?>
              <div id="man-company-help" class="form-text text-danger"><?php echo lang('NO DATA', $lang_file) ?></div>
            <?php } ?>
          </div>
          <!-- end company name -->
          <!-- start device name -->
          <div class="mb-3 form-floating form-floating-<?php echo $_SESSION['sys']['lang'] == 'ar' ? 'right' : 'left' ?>">
            <input type="text" class="form-control" id="device-name" name="device-name" placeholder="<?php echo lang('DEVICE NAME', $lang_file) ?>" autocomplete="off" required />
      ...
      <div class="modal-footer">
        <button type="button" class="btn btn-primary py-1 px-5 fs-12" form="addDevice" onclick="form_validation(this.form, 'submit')"><?php echo lang("ADD") ?></button>
