...
<div class="container" dir="<?php echo $page_dir ?>">
  <div class="mb-3 hstack gap-3">
    <?php if ($_SESSION['sys']['pcs_add'] == 1) { ?>
      <button type="button" class="btn btn-outline-primary py-1 fs-12" data-bs-toggle="modal"
        data-bs-target="#addNewDeviceModel">
  ...
  if (!empty($device_id) && $is_exist) {
    ...
    if ($device_counter > 0) { ?>
      <!-- start form -->
      <form class="mb-3 custom-form need-validation" action="?do=devices-companies&action=update-device" method="POST"
        id="editDeviceInfo">
        <!-- horzontal stack -->
        ...
        <div class="mb-3 row row-cols-sm-1 row-cols-md-2 g-3 align-items-stretch justify-content-start">
          ...
          <div class="col-12">
            <div class="section-block">
              ...
              <!-- device id -->
              <input type="hidden" class="form-control" id="device-id" name="device-id"
                value="<?php echo base64_encode($device_data['device_id']); ?>" placeholder="ID" autocomplete="off" />
            ...
            <div class="section-block">
              ...
              <div
                ...
                <input type="text" class="form-control" id="device-name" name="device-name"
                  value="<?php echo $device_data['device_name']; ?>"
        ...
        <div class="hstack gap-2">
          <?php if ($_SESSION['sys']['pcs_add'] == 1) { ?>
            <!-- save changes button -->
            <button type="button" form="editDeviceInfo"
              class="btn btn-primary text-capitalize bg-gradient fs-12 p-1 <?php echo @$_SESSION['sys']['lang'] == 'ar' ? 'me-auto' : 'ms-auto' ?>"
              id="edit-device-info" onclick="form_validation(this.form, 'submit')">
              <i class="bi bi-check-all"></i>
              <?php echo lang('SAVE'); ?>
            </button>
          <?php } ?>
          <!-- delete device info -->
          <button type="button" class="btn btn-outline-danger text-capitalize bg-gradient fs-12 p-1" data-bs-toggle="modal"
            data-bs-target="#deleteDeviceModal" data-id="<?php echo $device_data['device_id'] ?>"
            data-name="<?php echo $device_data['device_name'] ?>"
            onclick="put_data_into_modal(this, 'delete', 'deleted-device-id', 'deleted-device-name');">
                      ...
                      <tr>
                        ...
                        <td>
                          ...
                          if ($_SESSION['sys']['user_update'] == 1) { ?>
                            <a
                              href="<?php echo $nav_up_level ?>users/index.php?do=edit-user-info&userid=<?php echo base64_encode($model['added_by']) ?>">
                        ...
                        <td>
                          ...
                          <button type="button" class="btn btn-outline-success py-1 fs-12" data-bs-toggle="modal"
                            data-bs-target="#editDeviceModel" data-name="<?php echo $model['model_name'] ?>"
                            data-id="<?php echo base64_encode($model['model_id']) ?>"
                            onclick="put_data_into_modal(this, 'edit', 'model-id', 'old-model-name')">
                            <i class="bi bi-pencil-square"></i>
                            <?php echo lang('EDIT') ?>
                          </button>
                          <!-- edit button -->
                          <button type="button" class="btn btn-outline-danger py-1 fs-12" data-bs-toggle="modal"
                            data-bs-target="#deleteDeviceModel" data-name="<?php echo $model['model_name'] ?>"
                            data-id="<?php echo base64_encode($model['model_id']) ?>"
                            onclick="put_data_into_modal(this, 'delete', 'deleted-model-id', 'deleted-model-name')">
                            <i class="bi bi-trash"></i>
