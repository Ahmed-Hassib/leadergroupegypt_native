  ...
  <div class="modal-dialog modal-dialog-centered modal-model-dialog" dir="<?php echo $page_dir ?>">
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="m-auto modal-title h5 " id="staticBackdropLabel"><?php echo lang("DELETE MODEL INFO", $lang_file) ?></h5>
      </div>
      <div class="modal-body">
        <form action="?do=devices-companies&action=delete-model&back=true" method="POST" id="delete-model">
          ...
          <!-- start model name -->
          <div class="mb-3">
            <h4 class="h4"><?php echo lang('CONFIRM DELETE') ?>&nbsp;`<span class="text-danger" id="deleted-model-name"></span>`&nbsp;<?php echo @$_SESSION['sys']['lang'] == 'ar' ? '؟' : '?' ?></h4>
          </div>
          <!-- end model name -->
        </form>
      </div>
      <div class="modal-footer">
        <button type="button" class="btn btn-danger py-1 px-5 fs-12" form="delete-model" onclick="form_validation(this.form, 'submit')"><?php echo lang("DELETE") ?></button>
