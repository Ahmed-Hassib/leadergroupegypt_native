    ...
    <form class="custom-form need-validation" action="?name=<?php echo $page_title ?>&do=update-piece-info" method="POST" id="update-piece-info" onchange="form_validation(this)">
      <div class="hstack gap-3">
        <?php if ($_SESSION['pcs_update'] == 1) { ?>
          <!-- submit -->
          <button type="button" form="update-piece-info" class="btn btn-primary text-capitalize bg-gradient fs-12 p-1 <?php echo @$_SESSION['systemLang'] == 'ar' ? 'me-auto' : 'ms-auto' ?>" id="edit-piece" onclick="form_validation(this.form, 'submit')">
        ...
        <?php if ($target_user != -1) { ?>
          <a class="btn btn-outline-primary fs-12 w-auto py-1 px-2" href="?do=prepare-ip&id=<?php echo base64_encode($target_user['.id']) ?>&address=<?php echo $piece_data['ip'] ?>&port=443" target='_blank'><?php echo language('VISIT DEVICE', @$_SESSION['systemLang']) ?></a>
        <?php } ?>
                        ...
                        <select class="form-select" id="alternative-sources" name="alt-source-id">
                          <?php if ($sources_count) { ?>
                            ...
                            <?php foreach ($sources_data as $alt_source) { ?>
                              <option value="<?php echo $alt_source['id'] ?>" <?php if ($piece_data['alt_source_id'] == $alt_source['id'] || ($piece_data['alt_source_id'] == 0 && $piece_data['ip'] == $alt_source['ip'])) echo 'selected'; ?>>
