...
class Mysqldump
{
    ...
    public function __construct(
        ...
        $dumpSettingsDefault = array(
            'include_once-tables' => array(),
            'exclude-tables' => array(),
            'include_once-views' => array(),
        ...
        if (!is_array($this->dumpSettings['include_once-tables']) ||
            !is_array($this->dumpSettings['exclude-tables'])) {
            throw new Exception("Include-tables and exclude-tables should be arrays");
        }

        // If no include_once-views is passed in, dump the same views as tables, mimic mysqldump behaviour.
        if (!isset($dumpSettings['include_once-views'])) {
            $this->dumpSettings['include_once-views'] = $this->dumpSettings['include_once-tables'];
        }
    ...
    public function start($filename = '')
    {
        ...
        // If there still are some tables/views in include_once-tables array,
        // that means that some tables or views weren't found.
        // Give proper error and exit.
        // This check will be removed once include_once-tables supports regexps.
        if (0 < count($this->dumpSettings['include_once-tables'])) {
            $name = implode(",", $this->dumpSettings['include_once-tables']);
    ...
    private function getDatabaseStructureTables()
    {
        ...
        if (empty($this->dumpSettings['include_once-tables'])) {
            // include_once all tables for now, blacklisting happens later
            foreach ($this->dbHandler->query($this->typeAdapter->show_tables($this->dbName)) as $row) {
                array_push($this->tables, current($row));
            }
        } else {
            // include_once only the tables mentioned in include_once-tables
            foreach ($this->dbHandler->query($this->typeAdapter->show_tables($this->dbName)) as $row) {
                if (in_array(current($row), $this->dumpSettings['include_once-tables'], true)) {
                    array_push($this->tables, current($row));
                    $elem = array_search(
                        current($row),
                        $this->dumpSettings['include_once-tables']
                    );
                    unset($this->dumpSettings['include_once-tables'][$elem]);
                }
            }
        }
        return;
    ...
    private function getDatabaseStructureViews()
    {
        ...
        if (empty($this->dumpSettings['include_once-views'])) {
            // include_once all views for now, blacklisting happens later
            ...
            // include_once only the tables mentioned in include_once-tables
            foreach ($this->dbHandler->query($this->typeAdapter->show_views($this->dbName)) as $row) {
                if (in_array(current($row), $this->dumpSettings['include_once-views'], true)) {
                    array_push($this->views, current($row));
                    $elem = array_search(
                        current($row),
                        $this->dumpSettings['include_once-views']
                    );
                    unset($this->dumpSettings['include_once-views'][$elem]);
                }
            }
        }
        return;
