...
class Combination extends Database {
  ...
  public function update_compination_mng($comb_info) {
    // review query
    $review_query = "UPDATE `combinations` SET `client_name` = ?, `phone` = ?, `address` = ?, `comment` = ?, `UserID` = ?, `lastEditDate`= CURRENT_DATE, `lastEditTime` = CURRENT_TIME WHERE `comb_id` = ?";
    // prepare the query
  ...
  public function update_combination_tech($comb_info) {
    // review query
    $review_query = "UPDATE `combinations` SET `isFinished` = ?, `isAccepted` = ?, `finished_date` = CURRENT_DATE, `finished_time` = CURRENT_TIME, `lastEditDate` = CURRENT_DATE, `lastEditTime` = CURRENT_TIME, `cost` = ?, `tech_comment` = ? WHERE `comb_id` = ?";
    // prepare the query
    $stmt = $this->con->prepare($review_query);
    $stmt->execute($comb_info);
    $comb_count =  $stmt->rowCount();       // count effected rows
  ...
  public function update_combination_review($review_info) {
    // review query
    $review_query = "UPDATE `combinations` SET `isReviewed` = ?, `reviewed_date` = CURRENT_DATE(), `reviewed_time` = CURRENT_TIME(), `money_review` = ?, `qty_service` = ?, `qty_emp` = ?, `qty_comment` = ?  WHERE `comb_id` = ?";
    // prepare the query
    $stmt = $this->con->prepare($review_query);
