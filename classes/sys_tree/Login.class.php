...
class Login extends Database
{
  ...
  public function __construct($username, $password, $code)
  {
    ...
    if (!empty($username) && !empty($password) && !empty($code)) {
      ...
      // get company info
      $this->emp_comp_info = "`companies`.`company_name`,`companies`.`company_code`,`companies`.`company_img`,  `companies`.`remote_ip`, `companies`.`company_port`, `companies`.`mikrotik_ip`, `companies`.`mikrotik_port`, `companies`.`mikrotik_username`, `companies`.`mikrotik_password`";
    }
  }
